...
SELECT * FROM type_signalement;

-- Table type_sol
SELECT * FROM type_sol;

-- Table categorie
SELECT * FROM categorie;

-- Table date_action
SELECT * FROM date_action;

-- Table type_action
SELECT * FROM type_action;

-- Table date_plantation
SELECT * FROM date_plantation;

-- Table date_recolte
SELECT * FROM date_recolte;
...
SELECT produit.id_produit, produit.libelle_produit, produit.periode_plantation_optimale, produit.periode_recolte_optimale
FROM produit
JOIN categorie ON produit.categorie_id = categorie.id_categorie
WHERE categorie.libelle_categorie = 'fruit';

SELECT produit.id_produit, produit.libelle_produit, produit.periode_plantation_optimale, produit.periode_recolte_optimale
FROM produit
JOIN categorie ON produit.categorie_id = categorie.id_categorie
WHERE categorie.libelle_categorie = 'légume';

SELECT signalement.id_signalement, signalement.descriptif, signalement.photo, signalement.date_signalement,
       adherent.nom AS nom_adherent, adherent.adresse, adherent.numero_de_telephone,
       type_signalement.libelle_type_signalement
FROM signalement
JOIN adherent ON signalement.adherent_id = adherent.id_adherent
JOIN type_signalement ON signalement.type_signalement_id = type_signalement.id_type_signalement;

SELECT action.id_action, action.libelle_action,
       parcelle.id_parcelle, parcelle.largeur, parcelle.longueur,
       type_action.libelle_type_action,
       date_action.date_action
FROM action
JOIN parcelle ON action.parcelle_id = parcelle.id_parcelle
JOIN type_action ON action.type_action_id = type_action.id_type_action
JOIN date_action ON action.date_action_id = date_action.id_date_action;

-- Pour les plantations
SELECT est_plante.parcelle_id, parcelle.largeur, parcelle.longueur,
       est_plante.produit_id, produit.libelle_produit,
       date_plantation.date_plantation
FROM est_plante
JOIN parcelle ON est_plante.parcelle_id = parcelle.id_parcelle
JOIN produit ON est_plante.produit_id = produit.id_produit
JOIN date_plantation ON est_plante.date_plantation_id = date_plantation.id_date_plantation;


SELECT est_recolte.parcelle_id, parcelle.largeur, parcelle.longueur,
       est_recolte.produit_id, produit.libelle_produit,
       date_recolte.date_recolte, est_recolte.quantite_recoltee
FROM est_recolte
JOIN parcelle ON est_recolte.parcelle_id = parcelle.id_parcelle
JOIN produit ON est_recolte.produit_id = produit.id_produit
JOIN date_recolte ON est_recolte.date_recolte_id = date_recolte.id_date_recolte;


SELECT produit.id_produit, produit.libelle_produit, produit.periode_plantation_optimale, produit.periode_recolte_optimale,
       categorie.id_categorie, categorie.libelle_categorie
FROM produit
JOIN categorie ON produit.categorie_id = categorie.id_categorie;